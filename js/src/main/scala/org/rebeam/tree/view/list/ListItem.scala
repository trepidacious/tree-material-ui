...
//    .build
//
//  def apply(avatar: VdomElement, content: VdomElement, iconButtons: List[ButtonAction], onClick: Callback = Callback.empty, onClickAvatar: Callback = Callback.empty, onClickContents: Callback = Callback.empty, buttonBackgroundColor: Color = MaterialColor.Indigo(500)): ReactComponentU[Props, State, Backend, TopNode] =
...
//
//  def listItemWithEditAndDelete[A](name: String, firstLine: A => String, secondLine: A => String, avatar: A => VdomElement) = cursorView[A, EditAndDeleteActions](name){
//    cp => {
//      val m = cp.model
//
//      val buttons = List(
...
//  }
//
//  def listItemWithCompleteEditAndDelete[A](name: String, firstLine: A => String, secondLine: A => String, completionLens: LensN[A, Boolean]) = cursorView(name){
//    cp => {
//      val m = cp.model
//
//      val avatar = <.div(
