...
object TodoTableViews {
  ...
  val todoView = cursorView[Todo, Unit]("TodoView") { c =>
    ...
    MuiTableRow(key = t.id.toString, style = js.Dynamic.literal("border-bottom" -> "0px"))(
      MuiTableRowColumn(style = js.Dynamic.literal("width" -> "40px"))(
        booleanViewUnlabelled(c.zoom(Todo.completed).label(""))
      ),
      MuiTableRowColumn(style = js.Dynamic.literal("width" -> "40px"))(
        t.id.toString()
      ),
      MuiTableRowColumn(style = js.Dynamic.literal("width" -> "100%"))(
        textViewPlainLabel(c.zoom(Todo.name).label("Name"))
      ),
      MuiTableRowColumn(
  ...
  val todoListTableView = cursorView[TodoList, Unit]("TodoListTableView") { c =>
    MuiTable(
      selectable = false
    )(
      ...
      MuiTableBody(
        ...
        c.zoom(TodoList.items).zoomAllMatches(todo => FindTodoById(todo.id)).map(todoView(_))
      )
    )
  }
  ...
  val todoListView = ServerRootComponent[TodoList](noTodoList, "api/todolist") {
    c => {
      <.div(
        ^.margin := "24px",
        <.h3("Todo List"),
        textView(c.zoom(TodoList.name).label("Name")),
